public class Gleitpunktzahl {
    ...
    public void normalisiere() {
    	...
    	//Mantisse soll mit Bits aufgefuellt werden
    	if(this.mantisse < Math.pow(2, sizeMantisse - 1)) {
    	...
    	//Es kann auch sein, dass Bits abgeschnitten werden sollen.
    	int letzteBit = 0;
    	while(this.mantisse >= Math.pow(2, sizeMantisse)) {
    		letzteBit = this.mantisse & 1;
    		this.mantisse >>= 1;
    		this.exponent++;
    	}
    	//Wir soll aufrunden wenn die letzte Bit(r(-1)) 1 ist.
    	if(letzteBit == 1) {
    		this.mantisse += 1;
    		if(this.mantisse == Math.pow(2, sizeMantisse)) {
        		this.normalisiere();
        	}
    	}
    	...
    	//[1,2).2^-o-1 wird hin zu 2^-o aufgerundet.
    	if( this.toDouble() >= Math.pow(2, -1 - expOffset) &&
    		this.toDouble() < 2 * Math.pow(2, -1 - expOffset) ) {
    		this.mantisse = (int) Math.pow(2, sizeMantisse-1);
    		this.exponent = 0;
    	}
    ...
    public Gleitpunktzahl add(Gleitpunktzahl r) {
    	Gleitpunktzahl ergebnis = new Gleitpunktzahl();
    	
    	//Sonderfaelle
    	if(this.isInfinite() && r.isInfinite()) {
    		if(this.vorzeichen == r.vorzeichen)
    			ergebnis.setInfinite(vorzeichen);
    		else
    			ergebnis.setNaN();
    		return ergebnis;
    	} else if(r.isInfinite()) {
    		ergebnis.setInfinite(r.vorzeichen);
    		return ergebnis;
    	} else if(this.isInfinite()) {
    		ergebnis.setInfinite(this.vorzeichen);
    		return ergebnis;
    	}
        ...
        if(this.vorzeichen == r.vorzeichen) {
        	ergebnis.mantisse = this.mantisse + r.mantisse;
        	ergebnis.vorzeichen = this.vorzeichen;
        }
        else {
        	ergebnis.mantisse = this.mantisse > r.mantisse ? 
        			this.mantisse - r.mantisse :
        			r.mantisse - this.mantisse;
        	ergebnis.vorzeichen = this.mantisse > r.mantisse ? 
        			this.vorzeichen : r.vorzeichen;
        }
        
        ergebnis.exponent = this.exponent;
        
        ergebnis.normalisiere();
    ...
    public Gleitpunktzahl sub(Gleitpunktzahl r) {
    	...
    	Gleitpunktzahl r_2 = new Gleitpunktzahl(r);
    	r_2.vorzeichen = !r.vorzeichen;
    	...
    	if(this.isInfinite() && r.isInfinite()) {
    		if(this.vorzeichen == r.vorzeichen)
    			...
    			if(!this.vorzeichen && r.vorzeichen)
    				ergebnis.setInfinite(false);
    			else if(this.vorzeichen && !r.vorzeichen)
    				ergebnis.setInfinite(true);
    			else
    				ergebnis.setNaN();
    		return ergebnis;
    	} else if(r.isInfinite()) {
